...
VERSION: 67.0 (+ THEME CUSTOMIZATION)

НОВОЕ:
- Кнопка "🎨 Тема" в TopPanel
- Диалог настройки цветов (фон, кнопки, акцент, текст и т.д.)
- Тема сохраняется в theme.json и применяется при запуске
- Кнопки "Применить", "Сброс", "Сохранить"
"""
...
DEFAULT_THEME = {
    "accent":           "#3390ec",   # акцентный цвет (кнопки топ-панели, выбранная вкладка)
    "accent_hover":     "#2b82d9",   # кнопки при наведении
    "btn_tool_bg":      "#f1f1f1",   # фон кнопок панели инструментов
    "btn_tool_text":    "#3390ec",   # текст кнопок панели инструментов
    "btn_tool_hover":   "#e1f0ff",   # фон кнопок инструментов при наведении
    "top_bg":           "#ffffff",   # фон TopPanel и ToolsFrame
    "top_border":       "#dfe5ec",   # граница панелей
    "editor_bg":        "#f0f2f5",   # фон редактора
    "tab_bg":           "#ffffff",   # фон вкладок
    "tab_text":         "#707579",   # текст неактивных вкладок
    "tab_selected_text":"#3390ec",   # текст активной вкладки
    "search_bg":        "#f1f1f1",   # фон строки поиска
    "search_text":      "#222222",   # текст поиска
    "bubble_bg":        "#ffffff",   # фон пузырей текста
    "bubble_border":    "#dfe5ec",   # граница пузырей
    "window_bg":        "#f0f2f5",   # фон главного окна
}

THEME_LABELS = {
    "accent":           "Акцент (кнопки, вкладка)",
    "accent_hover":     "Акцент при наведении",
    "btn_tool_bg":      "Фон кнопок инструментов",
    "btn_tool_text":    "Текст кнопок инструментов",
    "btn_tool_hover":   "Кнопки инструментов (hover)",
    "top_bg":           "Фон верхней панели",
    "top_border":       "Граница панелей",
    "editor_bg":        "Фон редактора",
    "tab_bg":           "Фон вкладок",
    "tab_text":         "Текст вкладок",
    "tab_selected_text":"Текст активной вкладки",
    "search_bg":        "Фон строки поиска",
    "search_text":      "Текст поиска",
    "bubble_bg":        "Фон пузырей",
    "bubble_border":    "Граница пузырей",
    "window_bg":        "Фон окна",
}
...
def build_stylesheet(t):
    return f"""
        ...
        #TopPanel, #ToolsFrame {{ background-color: {t['top_bg']}; border-bottom: 1px solid {t['top_border']}; }}
        QTabBar::tab {{ background: {t['tab_bg']}; color: {t['tab_text']}; padding: 12px 25px; border-right: 1px solid #f1f1f1; min-width: 140px; }}
        QTabBar::tab:selected {{ background: {t['tab_bg']}; color: {t['tab_selected_text']}; border-bottom: 3px solid {t['tab_selected_text']}; font-weight: bold; }}
        QTextEdit {{ background-color: {t['editor_bg']}; border: none; padding: 10px; }}
        QLineEdit {{ background-color: {t['search_bg']}; border-radius: 15px; border: 1px solid {t['top_border']}; padding: 10px 15px; color: {t['search_text']}; font-size: 14px; }}
        QPushButton {{
            background-color: {t['accent']}; color: white; border-radius: 12px;
            padding: 10px 20px; font-weight: bold; border: none; min-height: 45px;
        }}
        QPushButton:hover {{ background-color: {t['accent_hover']}; }}
        #SearchNav {{ background-color: {t['accent']}; color: white; border-radius: 20px; font-size: 20px; padding: 0; min-height: 40px; }}
        #ToolsFrame QPushButton {{
            background-color: {t['btn_tool_bg']}; color: {t['btn_tool_text']}; border: 1px solid {t['top_border']};
            padding: 10px 20px; min-width: 125px; min-height: 42px; font-size: 14px;
        }}
        #ToolsFrame QPushButton:hover {{ background-color: {t['btn_tool_hover']}; }}
    """

# --- ДИАЛОГ НАСТРОЙКИ ТЕМЫ ---

class ThemeDialog(QDialog):
    def __init__(self, current_theme, apply_callback, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🎨 Настройка темы")
        self.resize(520, 540)  # 👈 компактная высота
        self.theme = dict(current_theme)
        ...
        # Заголовок
        title = QLabel("<b style='font-size:16px;'>🎨 Настройка цветов интерфейса</b>")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        main_l.addWidget(title)

        # Пресеты
        presets_frame = QFrame()
        presets_frame.setStyleSheet("background:#f8f9fa; border-radius:10px; padding:5px;")
        pl = QHBoxLayout(presets_frame)
        pl.addWidget(QLabel("Пресеты:"))

        for name, theme_data in self._presets().items():
            btn = QPushButton(name)
            btn.setFixedHeight(30)
            btn.setStyleSheet("min-height:30px; padding:4px 10px;")
            btn.clicked.connect(lambda _, td=theme_data: self._apply_preset(td))
            pl.addWidget(btn)

        main_l.addWidget(presets_frame)

        # --- КОМПАКТНАЯ СЕТКА (2 КОЛОНКИ) ---
        group = QGroupBox("Цвета")
        grid = QGridLayout(group)
        grid.setSpacing(6)

        items = list(THEME_LABELS.items())
        rows = (len(items) + 1) // 2
        ...
        for i, (key, label) in enumerate(items):
            ...
            color_btn.setFixedSize(36, 26)
            color_btn.setStyleSheet(
                f"background-color: {self.theme[key]}; border-radius: 6px; border: 1px solid #ccc;"
            )
            color_btn.clicked.connect(lambda _, k=key: self._pick_color(k))

            hex_edit = QLineEdit(self.theme[key])
            hex_edit.setFixedWidth(85)
            hex_edit.setStyleSheet("padding: 3px 6px; border-radius: 6px;")
            hex_edit.textChanged.connect(lambda text, k=key: self._hex_changed(k, text))

            self.color_buttons[key] = (color_btn, hex_edit)

            grid.addWidget(lbl,       row, base_col)
            grid.addWidget(color_btn, row, base_col + 1)
            grid.addWidget(hex_edit,  row, base_col + 2)

        main_l.addWidget(group)
        ...
        btn_apply.setStyleSheet("background:#3390ec; color:white; min-height:36px;")
        btn_save.setStyleSheet("background:#27ae60; color:white; min-height:36px;")
        btn_reset.setStyleSheet("background:#e74c3c; color:white; min-height:36px;")
        btn_close.setStyleSheet("background:#888; color:white; min-height:36px;")

        btn_apply.clicked.connect(self._do_apply)
        btn_save.clicked.connect(self._do_save)
        btn_reset.clicked.connect(self._do_reset)
        btn_close.clicked.connect(self.accept)

        btn_l.addWidget(btn_apply)
    ...
    def _presets(self):
        return {
            "🔵 Telegram": DEFAULT_THEME,
            "🌙 Тёмная": {
                "accent": "#7289da", "accent_hover": "#5b6eae",
                "btn_tool_bg": "#2c2f33", "btn_tool_text": "#7289da",
                "btn_tool_hover": "#3a3d42", "top_bg": "#23272a",
                "top_border": "#444444", "editor_bg": "#1e2124",
                "tab_bg": "#23272a", "tab_text": "#99aab5",
                "tab_selected_text": "#7289da", "search_bg": "#2c2f33",
                "search_text": "#ffffff", "bubble_bg": "#2c2f33",
                "bubble_border": "#444444", "window_bg": "#1e2124",
            },
            "🌿 Зелёная": {
                "accent": "#27ae60", "accent_hover": "#219a52",
                "btn_tool_bg": "#f0faf4", "btn_tool_text": "#27ae60",
                "btn_tool_hover": "#d5f0e0", "top_bg": "#ffffff",
                "top_border": "#b2dfcc", "editor_bg": "#f4faf7",
                "tab_bg": "#ffffff", "tab_text": "#6c757d",
                "tab_selected_text": "#27ae60", "search_bg": "#edf7f1",
                "search_text": "#222222", "bubble_bg": "#ffffff",
                "bubble_border": "#b2dfcc", "window_bg": "#f4faf7",
            },
            "🟠 Тёплая": {
                "accent": "#e67e22", "accent_hover": "#d35400",
                "btn_tool_bg": "#fff8f0", "btn_tool_text": "#e67e22",
                "btn_tool_hover": "#fdebd0", "top_bg": "#ffffff",
                "top_border": "#f0c08a", "editor_bg": "#fdf6ec",
                "tab_bg": "#ffffff", "tab_text": "#707579",
                "tab_selected_text": "#e67e22", "search_bg": "#fff8f0",
                "search_text": "#333333", "bubble_bg": "#ffffff",
                "bubble_border": "#f0c08a", "window_bg": "#fdf6ec",
            },
        }

    def _apply_preset(self, theme_data):
        self.theme = dict(DEFAULT_THEME)
        self.theme.update(theme_data)
        self._refresh_buttons()

    def _refresh_buttons(self):
        for key, (color_btn, hex_edit) in self.color_buttons.items():
            color_btn.setStyleSheet(
                f"background-color: {self.theme[key]}; border-radius: 6px; border: 1px solid #ccc;"
            )
            hex_edit.blockSignals(True)
            hex_edit.setText(self.theme[key])
    ...
    def _hex_changed(self, key, text):
        if len(text) == 7 and text.startswith("#"):
            try:
                QColor(text)
                self.theme[key] = text
                btn, _ = self.color_buttons[key]
                btn.setStyleSheet(
                    f"background-color: {text}; border-radius: 6px; border: 1px solid #ccc;"
                )
            except:
                pass
    ...
    def _do_save(self):
        ...
        QMessageBox.information(self, "Сохранено", "Тема сохранена.")
...
class ClientLogPro(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ClientLog Pro v67.0")
        self.resize(1200, 850); self.reminders = []
        self.theme = load_theme()

        cw = QWidget(); self.setCentralWidget(cw)
        self.main_l = QVBoxLayout(cw); self.main_l.setContentsMargins(0,0,0,0); self.main_l.setSpacing(0)
        ...
        self.theme_b = QPushButton("🎨 Тема"); self.theme_b.clicked.connect(self.open_theme_dialog)
        ...
        tl.addWidget(self.add_b); tl.addWidget(self.open_b); tl.addWidget(self.theme_b)
        tl.addSpacing(20)
        ...
        self.tabs.tabCloseRequested.connect(self.close_tab_request)
    ...
    def open_theme_dialog(self):
        dlg = ThemeDialog(self.theme, self.apply_theme, self)
        dlg.exec()

    def add_entry(self, ed):
        now = datetime.datetime.now().strftime("%d.%m.%Y %H:%M")
        t = self.theme
        header = f'<div style="color: {t["accent"]}; font-weight: bold; margin-top: 20px; margin-left: 10px; margin-bottom: 5px;">📍 {now}</div>'
        bubble = (
            f'<table width="50%" style="margin-bottom: 10px;">'
            f'<tr><td style="background-color: {t["bubble_bg"]}; border: 1px solid {t["bubble_border"]}; border-radius: 12px; '
